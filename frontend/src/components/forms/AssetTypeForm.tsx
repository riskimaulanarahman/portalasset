...
interface AssetTypeFormProps {
  initialData?: any;
  onSubmit: (data: AssetTypeFormValues) => void;
  onCancel: () => void;
}

const AssetTypeForm: React.FC<AssetTypeFormProps> = ({ initialData, onSubmit, onCancel }) => {
  const { register, handleSubmit, formState: { errors, isSubmitting } } = useForm<AssetTypeFormValues>({
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      <div>
        ...
        <input
          ...
          placeholder="e.g. Laptop, Excavator, Printer"
        />
